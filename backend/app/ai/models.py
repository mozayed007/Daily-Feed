...
class SummaryResult(BaseModel):
    ...
    summary: str = Field(description="Concise, accurate summary of the article in 2-4 sentences")
    category: str = Field(
...
class CritiqueResult(BaseModel):
    ...
    bias: int = Field(description="Lack-of-bias score 1-10 (higher = less biased)", ge=1, le=10)
...
class ArticleCluster(BaseModel):
    ...
    confidence: float = Field(description="Clustering confidence 0.0-1.0", ge=0, le=1)
...
class MultiSourceSynthesis(BaseModel):
    ...
    consensus_points: List[str] = Field(description="Points where all or most sources agree")
...
class DigestReasoning(BaseModel):
    ...
    relevance_score: float = Field(description="Personalized relevance score 0.0-1.0", ge=0, le=1)
...
class TrendResult(BaseModel):
    ...
    top_sources: List[str] = Field(description="Sources that most actively cover this trend")
