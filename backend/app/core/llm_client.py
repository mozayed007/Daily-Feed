...
class OllamaClient(BaseLLMClient):
    ...
    async def generate(
        ...
        try:
            ...
            response = await self.client.generate(
                model=self.model, prompt=prompt, system=system or "", options=options
            )
    ...
    async def chat(
        ...
        try:
            ...
            response = await self.client.chat(model=self.model, messages=messages, options=options)

            return LLMResponse(text=response["message"]["content"], model=self.model)
        except Exception as e:
