...
class SimpleMemStore:
    ...
    def synthesize(
        self, unit_ids: List[str], consolidated_content: str, reason: str = "semantic_similarity"
    ) -> MemoryUnit:
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            for uid in unit_ids:
                row = conn.execute("SELECT * FROM memory_units WHERE id = ?", (uid,)).fetchone()
                if row:
                    sources.append(row)
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (json.dumps(unit_ids), new_unit.id, datetime.now(timezone.utc).isoformat(), reason),
            )
            conn.commit()
    ...
    def get_recent(self, days: int = 7, category: Optional[str] = None) -> List[MemoryUnit]:
        """Get recent memories."""
        return self.retrieve(time_range_days=days, category=category, limit=50)
...
class ArticleMemory(SimpleMemStore):
    ...
    def find_similar_articles(
        self, title: str, category: Optional[str] = None, limit: int = 5
    ) -> List[MemoryUnit]:
    ...
    def get_user_interests(self) -> Dict[str, float]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for row in rows:
                ...
                score = row["count"] * 0.3 + row["total_access"] * 0.5 + row["avg_importance"] * 20
