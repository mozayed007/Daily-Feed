...
class PersonalizationEngine:
    ...
    def __init__(self, weights: Optional[Dict[str, float]] = None):
        ...
        logger.info("personalization_engine_initialized", weights=self.weights)
    ...
    def _calculate_score(
        self, article: ArticleModel, preferences: UserPreferencesModel
    ...
    def _calculate_freshness_score(
        ...
        age = (
            datetime.now(timezone.utc) - article.published_at.replace(tzinfo=timezone.utc)
            if article.published_at.tzinfo is None
            else datetime.now(timezone.utc) - article.published_at
        )
    ...
    def filter_articles(
        self, articles: List[ArticleModel], preferences: UserPreferencesModel
    ) -> List[ArticleModel]:
...
class UserModelTrainer:
    ...
    def update_from_interaction(
        ...
        positive_signals = self._calculate_positive_signals(rating, read_duration, saved)
        negative_signals = self._calculate_negative_signals(rating, read_duration, dismissed)
    ...
    def _calculate_positive_signals(self, rating: int, read_duration: int, saved: bool) -> float:
    ...
    def _calculate_negative_signals(
        self, rating: int, read_duration: int, dismissed: bool
    ) -> float:
    ...
    def _update_topic_interest(
        self, preferences: UserPreferencesModel, topic: str, delta: float
    ) -> float:
    ...
    def _update_source_preference(
        self, preferences: UserPreferencesModel, source: str, delta: float
    ) -> float:
    ...
    def decay_old_interests(
        self, preferences: UserPreferencesModel, days: int = 30, decay_rate: float = 0.95
    ):
