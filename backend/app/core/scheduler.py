    ...
    def get_next_run(cron_expr: str, after: Optional[datetime] = None) -> datetime:
        ...
        for _ in range(365 * 24 * 60):
            ...
            if (
                ...
                and cron_weekday in fields["dow"]
            ):
                return candidate
...
def cron(cron_expr: str, name: Optional[str] = None):
    ...
    def decorator(func: Callable):
        ...
        get_scheduler().add_cron_job(name=job_name, cron=cron_expr, callback=func)
        return func
...
def interval(seconds: int, name: Optional[str] = None):
    ...
    def decorator(func: Callable):
        job_name = name or func.__name__
        get_scheduler().add_interval_job(name=job_name, seconds=seconds, callback=func)
        return func
