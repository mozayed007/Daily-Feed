...
from app.core.memory import ArticleMemory, get_memory_store
from app.core.tool_base import Tool, ToolResult
from app.database import ArticleModel, Database
...
class MemoryTool(Tool):
    ...
    async def _remember_article(self, article_id: Optional[int]) -> ToolResult:
        ...
        if not article_id:
            return ToolResult(
                success=False, data=None, error="article_id required for remember_article"
            )
        ...
        async with Database.get_session() as db:
            ...
            if not article:
                return ToolResult(success=False, data=None, error=f"Article {article_id} not found")
            ...
            return ToolResult(
                success=True,
                data={"memory_id": unit.id, "article_id": article_id, "category": unit.category},
                message=f"Article '{article.title[:40]}...' remembered in category '{unit.category}'",
            )

    def _find_similar(
        self, title: Optional[str], category: Optional[str], limit: int
    ) -> ToolResult:
        """Find similar articles in memory."""
        if not title:
            return ToolResult(success=False, data=None, error="title required for find_similar")
    ...
    def _get_interests(self) -> ToolResult:
        ...
        return ToolResult(
            success=True,
            data={"interests": interests, "top_categories": list(interests.keys())[:5]},
