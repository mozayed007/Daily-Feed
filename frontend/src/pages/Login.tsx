    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-500 via-purple-500 to-pink-500 p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md p-6 sm:p-8">
        <div className="flex flex-col items-center mb-6 sm:mb-8">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm sm:text-base">Sign in to continue to Daily Feed</p>
        </div>
    ...
    try {
      ...
      const handleMessage = (event: MessageEvent) => {
        if (event.data?.type === 'oauth_callback') {
          ...
          // OAuthCallback page will handle the actual token exchange
        }
      };
      ...
      toast.addToast({
        type: 'error',
        title: 'OAuth Error',
        message: `Failed to initiate ${provider} login. Please try again.`,
        duration: 5000,
      });
    }
  };
  ...
  return (
    <div className="space-y-3">
      <div className="relative flex items-center justify-center">
        ...
        <span className="relative bg-white dark:bg-slate-900 px-4 text-xs text-slate-400 uppercase">
          Or continue with
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <button
          ...
          className="flex items-center justify-center gap-2 py-2.5 px-4 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors disabled:opacity-50"
        >
